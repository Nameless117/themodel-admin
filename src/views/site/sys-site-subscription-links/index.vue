...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                this.loading = true
                listSysSiteSubscriptionLinks(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
                        this.sysSiteSubscriptionLinksList = response.data.list
            ...
            handleUpdate(row) {
                ...
                const id =
                row.id || this.ids
                getSysSiteSubscriptionLinks(id).then(response => {
                ...
                this.$refs['form'].validate(valid => {
                    if (valid) {
                        if (this.form.id !== undefined) {
                            updateSysSiteSubscriptionLinks(this.form).then(response => {
                            ...
                            addSysSiteSubscriptionLinks(this.form).then(response => {
                                if (response.code === 200) {
                                    this.msgSuccess(response.msg)
                                    this.open = false
                                    this.getList()
            ...
            handleDelete(row) {
                ...
                this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
                    ...
                    type: 'warning'
                      ...
                      return delSysSiteSubscriptionLinks( { 'ids': Ids })
                }).then((response) => {
                   if (response.code === 200) {
                     this.msgSuccess(response.msg)
                     this.open = false
