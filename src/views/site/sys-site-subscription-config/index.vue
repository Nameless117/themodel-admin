                ...
                <template #tip>
                  ...
                  <div v-if="form.exampleUrl" class="upload-preview">
                    ...
                    <div class="file-info">
                      ...
                      <span class="file-name">{{ getFileName(form.exampleUrl) }}</span>
                    </div>
                  </div>
                  <!-- 添加 PDF 预览弹窗 -->
                  <el-dialog
...
export default {
  ...
  computed: {
    // 获取预览图URL
    previewPdfUrl() {
      if (!this.form.exampleUrl) return ''
      // 使用后端预览图生成接口，如果有的话
    ...
    fullPdfUrl() {
      if (!this.form.exampleUrl) return ''
      console.log(this.getFullPath(this.form.exampleUrl))
      return this.getFullPath(this.form.exampleUrl)
    }
  },
