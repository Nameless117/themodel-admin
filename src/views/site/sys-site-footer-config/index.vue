...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                this.loading = true
                listSysSiteFooterConfig(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
                        this.sysSiteFooterConfigList = response.data.list
            ...
            handleUpdate(row) {
                ...
                const id =
                row.id || this.ids
                getSysSiteFooterConfig(id).then(response => {
                    this.form = response.data
                    this.open = true
                    this.title = '修改站点Footer管理'
                    this.isEdit = true
                })
            },
            /** 提交按钮 */
                ...
                this.$refs['form'].validate(valid => {
                    if (valid) {
                        if (this.form.id !== undefined) {
                            updateSysSiteFooterConfig(this.form).then(response => {
                            ...
                            addSysSiteFooterConfig(this.form).then(response => {
                                if (response.code === 200) {
                                    this.msgSuccess(response.msg)
                                    this.open = false
                                    this.getList()
            ...
            handleDelete(row) {
                ...
                this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
                    ...
                    type: 'warning'
                      ...
                      return delSysSiteFooterConfig( { 'ids': Ids })
                }).then((response) => {
                   if (response.code === 200) {
                     this.msgSuccess(response.msg)
                     this.open = false
